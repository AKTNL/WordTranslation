...
- 🕊️ **克制优雅的划词触发（三大模式自由切换）**：
  - **轻量悬浮图标（默认推荐）**：划选后仅在光标旁浮现精致小图标，点击才展开释义卡片，绝不遮挡正文与打扰复制；
  - **划选直接秒弹**：适合习惯秒出的用户，搭配防抖判定；
...
### 1. 划词触发模式（可在扩展弹窗自由配置）
* **【轻量悬浮图标】（默认）**：划选英文文本后，光标右上方仅浮现一个半透明精致小蓝标，点击图标才展开查词卡片。
* **【划选直接秒弹】**：鼠标松开瞬间立即弹出卡片，适合追求极速查词的用户。
