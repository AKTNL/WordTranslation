...
- [🌟 核心功能与亮点](#-核心功能与亮点)
- [📖 论文就地全文双语与纯中文速读 (Phase 2 全新功能)](#-论文就地全文双语与纯中文速读-phase-2-全新功能)
- [🚀 极速安装与部署指南](#-极速安装与部署指南)
...
- ⚡ **全文就地双语速读与纯中文转换（全新 Phase 2）**：
  - 常驻右侧可折叠悬浮控制胶囊，支持快捷键 `Alt+B`；
  - 智能识别学术结构，保护 LaTeX / KaTeX / MathML 公式与代码块；
  - 视口懒加载（IntersectionObserver）随滚随译，极速流畅。

---

## 📖 论文就地全文双语与纯中文速读 (Phase 2 全新功能)

针对赶时间或希望直接以母语阅读英文论文的研究人员与学生，PaperDict 提供了**一键就地论文全文阅读转换**：

### 1. 三种阅读视图自由切换
* **【双语对照】（Bilingual View）**：
  在英文原版段落下方优雅插入浅灰底色的中文学术译文，原汁原味对照学习，学术术语与原句结构一目了然。
* **【纯享中文】（Full Chinese View）**：
  隐藏/淡化原英文段落，整篇论文瞬间呈现为流畅中文，文献粗读速度提升 3~5 倍！
* **【原版英文】（Original View）**：
  一键还原学术期刊或会议原排版，完全无侵入。

### 2. 悬浮阅读控制胶囊与全局快捷键
* 页面右边缘常驻半透明精致胶囊，悬停/点击可展开模式控制面板与翻译进度；
* 支持**全局快捷键 `Alt + B`**（Mac 上为 `Option + B`），在原版、双语、纯中文间一键平滑循环切换；
* 胶囊支持拖拽定位至任意舒适位置，双击头部可折叠为小药丸，绝不遮挡论文正文。

### 3. 学术公式与格式严苛保护 (Formula Protection)
* 自动识别与锁定文本及 DOM 中的数学公式（`$公式$`、`$$行间公式$$`、`<math>`、KaTeX、MathJax）；
* 翻译前自动将其替换为安全隔离占位符，译文生成后精确回填公式原始渲染结构，**100% 杜绝公式乱码与反斜杠丢失**。

### 4. 智能降噪与视口懒加载 (Smart Filtering & Viewport Lazy Loading)
* **智能降噪**：自动识别并跳过论文参考文献（References / Bibliography / Citations）、代码块（`<pre><code>`）、页眉页脚与导航元素，避免无意义翻译与版面污染。
* **视口懒加载**：采用 `IntersectionObserver` 仅翻译用户当前浏览视口内及附近的段落，滚动时动态流式加载，零等待零卡顿，节省网络与电量消耗。

### 5. PDF 阅读器双语联动
* 在 PaperDict 学术 PDF 阅读器中，顶部工具栏同样集成了【双语对照】与【纯享中文】切换开关；
* 阅读器采用左右分栏对齐设计，左侧保留原版高精排版，右侧实时流式对照呈现中文段落。
